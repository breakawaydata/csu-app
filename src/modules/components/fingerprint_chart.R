...
## Create the DF for the fingerprint chart 
  fingerprint_df <- data.frame(
    ...
    value = c(state$values$explosion_score,
              state$values$reach_score,
              state$values$balance_score))%>%
    mutate(dummy = category)
  
  len <- 2
  empty_df <- data.frame(category = letters[1:len], value = rep(0, len), 
                         dummy = rep("", len))
  ...
  output$fingerprint_plot <- 
    ...
    scale_fill_manual(
      ...
      values = c(
        ...
        " " = "white")) +
    theme_void() +
    geom_text(aes(label = overall_score, x = 0, y = 25),   # need to update font here:: family = "Axia Stencil Black"
              size= 12) +
